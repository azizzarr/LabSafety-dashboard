...
export class OrdersChartService {
  ...
  getOrdersChartData(period: string): Observable<OrdersChart> {
    return forkJoin([
      ...
      map(([allWorkers, cms1Workers]: [Worker[], Worker[]]) => {
        const allWorkersChargeEsdByDate: { [key: string]: number[] } = {};
        const cms1WorkersChargeEsdByDate: { [key: string]: number[] } = {};

        allWorkers.forEach(worker => {
          const dateString = new Date(worker.date[0], worker.date[1] - 1, worker.date[2]).toLocaleDateString();
          if (!allWorkersChargeEsdByDate[dateString]) {
            allWorkersChargeEsdByDate[dateString] = [];
          }
          allWorkersChargeEsdByDate[dateString].push(worker.chargeEsd);
        });

        cms1Workers.forEach(worker => {
          const dateString = new Date(worker.date[0], worker.date[1] - 1, worker.date[2]).toLocaleDateString();
          if (!cms1WorkersChargeEsdByDate[dateString]) {
            cms1WorkersChargeEsdByDate[dateString] = [];
          }
          cms1WorkersChargeEsdByDate[dateString].push(worker.chargeEsd);
        });

        const allWorkersChartLabel: string[] = Object.keys(allWorkersChargeEsdByDate);
        const allWorkersLinesData: number[][] = Object.values(allWorkersChargeEsdByDate).map((data: number[]) =>
          data.reduce((acc, val) => acc.concat(val), []));

        const cms1WorkersChartLabel: string[] = Object.keys(cms1WorkersChargeEsdByDate);
        const cms1WorkersLinesData: number[][] = Object.values(cms1WorkersChargeEsdByDate).map((data: number[]) =>
          data.reduce((acc, val) => acc.concat(val), []));

        const chartLabel = allWorkersChartLabel.concat
        (cms1WorkersChartLabel.filter(label => !allWorkersChartLabel.includes(label)));
        const linesData = allWorkersLinesData.concat
        (cms1WorkersLinesData.filter((_, index) => !allWorkersChartLabel.includes(cms1WorkersChartLabel[index])));

        // Sort the dates
        chartLabel.sort((a, b) => new Date(a).getTime() - new Date(b).getTime());

        // console.log('Chart Label:', chartLabel); // Log the chart label
       // console.log('Lines Data:', linesData); // Log the lines data

        return { chartLabel, linesData, dataStream: of(null) } as OrdersChart;
      })
    );
  }
