...
import { LoadingEmails } from '../store-alert/actions/email.actions';
import { EmailState } from '../store-alert/reducers/email.reducer';
...
/*
  ...
  private _createEmail(): { subject: string; tabs: (string)[]; name: string; isRead: boolean; savedAt: number; from: string; _id: string; to: string; body: string; labels: any[] } {
    ...
    const email = {
      ...
      tabs: isIncoming ? [Math.random() > .5 ? 'starred' : 'important', Math.random() > .3 ? 'Alerts' : 'spam'] : [Math.random() > .5 ? 'important' : 'starred', 'sent'],
