...
import { ADDED_EMAIL, LOADED_EMAIL, LoadEmail, LoadEmails, SaveEmail, UPDATED_EMAIL } from '../../store-alert/actions/email.actions'
...
export class EmailComposeComponent {
  ...
  email: Email = {status: undefined, tabs: [], to: '', subject: '', body: '' }
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(({ compose, re, fwd }) => {
      ...
      if (this.email._id !== compose) {
        if (compose === 'new') {
          this.email = {status: undefined, tabs: [], to: '', subject: '', body: '' }
  ...
  buildRefEmail(mode: string, email: Email): Email {
    ...
    return {status: undefined, tabs: [], to, subject, body }
  }
